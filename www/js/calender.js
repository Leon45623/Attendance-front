...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function update() {
	  ...
	  this.findmysheet($(".sc-select-year").val(),$(".sc-select-month").val(),{
...
'pwId':employee.pwId
    ...
    key: 'findmysheet',
    value: function findmysheet(year, month,msg) {
	data=JSON.stringify(msg);
	$.ajax({
				url:"http://"+"192.168.191.1"+":8080/Attendance/webservice/hehe/findsheet",
				type:"post",
				dataType:'json',
				data:data,
				...
				console.log(data);
          if(data!=null&&getLength(data)>0){

		   var daysElement = [];
      [].forEach.call(document.querySelector("#container").querySelectorAll('.sc-item'), function (v) {
        daysElement.push(v);
      });
      ...
      for (var i = 0; i < 35; i++) {
		...
		$.each(data,function(num,test){
		var arr=test.date.split("-");
	     var year=parseInt(arr[0]);
         var month=parseInt(arr[1]);
		 var day=parseInt(arr[2]);
		if(writeyear==year&&writeday==day&&writemonth==month){
		 daysElement[i].querySelector('.lunar-day').innerHTML = test.result;
